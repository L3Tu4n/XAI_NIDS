...
XAI_SEVERITY_THRESHOLD = int(os.getenv('XAI_SEVERITY_THRESHOLD', '80'))
...
class StreamAggregator:
    ...
    def extract_flow_features(self, meta, b_row):
        """
        ✅ FIX #2: Extract ML features for a single flow
        Used for both cached and non-cached flows
        """
        return {
            # Basic metrics
            ...
            'missed_bytes': int(safe_num(meta.get('missed_bytes', 0))),
            'orig_pkts': int(safe_num(meta.get('orig_pkts'))),
            'orig_ip_bytes': int(safe_num(meta.get('orig_ip_bytes', 0))),
            'resp_pkts': int(safe_num(meta.get('resp_pkts'))),
            'resp_ip_bytes': int(safe_num(meta.get('resp_ip_bytes', 0))),
            
            # Derived metrics
            'total_bytes': int(safe_num(meta.get('orig_bytes'))) + 
                          int(safe_num(meta.get('resp_bytes'))),
            'total_pkts': int(safe_num(meta.get('orig_pkts'))) + 
                         int(safe_num(meta.get('resp_pkts'))),
            
            # Rates (avoid division by zero)
            'bytes_per_sec': (
                (int(safe_num(meta.get('orig_bytes'))) + 
                 int(safe_num(meta.get('resp_bytes')))) /
                max(0.001, float(safe_num(meta.get('duration'))))
            ),
            'pkts_per_sec': (
                (int(safe_num(meta.get('orig_pkts'))) + 
                 int(safe_num(meta.get('resp_pkts')))) /
                max(0.001, float(safe_num(meta.get('duration'))))
            ),
            'bytes_per_pkt': (
                (int(safe_num(meta.get('orig_bytes'))) + 
                 int(safe_num(meta.get('resp_bytes')))) /
                max(1, int(safe_num(meta.get('orig_pkts'))) + 
                       int(safe_num(meta.get('resp_pkts'))))
            ),
            'byte_ratio': (
                int(safe_num(meta.get('orig_bytes'))) /
                max(1, int(safe_num(meta.get('resp_bytes'))))
            ),
            'pkt_ratio': (
                int(safe_num(meta.get('orig_pkts'))) /
                max(1, int(safe_num(meta.get('resp_pkts'))))
            ),
            
            # Protocol features
            'proto_numeric': int(safe_num(meta.get('proto_numeric', 0))),
            'proto_str': safe_str(meta.get('proto')),
            'service': safe_str(meta.get('service')),
            'conn_state': safe_str(meta.get('conn_state')),
            
            # Behavioral features
            'src_conn_count': int(safe_num(b_row.get('src_conn_count'))),
            'src_unique_dests': int(safe_num(b_row.get('src_unique_dests'))),
            'src_unique_ports': int(safe_num(b_row.get('src_unique_ports'))),
            'src_service_diversity': int(safe_num(b_row.get('src_service_diversity'))),
            'dst_conn_count': int(safe_num(b_row.get('dst_conn_count')))
        }
        ...
        try:
                ...
                if cached:
                    ...
                    final_results[i] = {
                        ...
                        'cache_metadata': {
                            ...
                            'last_seen': cached['last_seen'].isoformat()
                        }
                    }
                else:
                    # CACHE MISS
            ...
            for i, result in enumerate(final_results):
                ...
                if key not in attack_groups:
                    ...
                    clean_features = {
                        k: float(v) if isinstance(v, (np.float32, np.float64, np.integer)) else v 
                        for k, v in encoded_features_row.items()
                    }
                    ...
                    attack_groups[key] = {
                        ...
                        'representative_features': clean_features
                    }
                ...
                flow_record = {
                    ...
                    'dst_port': dst_port,
                    'confidence': result['confidence'],
                    'ml_features': current_flow_features
                }
                
                candidates = group['flow_candidates']
                
                # 1. Highest confidence
                if candidates['highest_confidence'] is None or \
                   result['confidence'] > candidates['highest_confidence']['confidence']:
                    candidates['highest_confidence'] = flow_record
                
                # 2. Median confidence candidates (limit to 100)
                if len(candidates['median_confidence']) < 100:
                    candidates['median_confidence'].append(flow_record)
                
                # 3. Highest volume
                volume = current_flow_features['total_bytes']
                if candidates['highest_volume'] is None or \
                   volume > candidates['highest_volume']['ml_features']['total_bytes']:
                    candidates['highest_volume'] = flow_record
                
                # 4. Longest duration
                duration = current_flow_features['duration']
                if candidates['longest_duration'] is None or \
                   duration > candidates['longest_duration']['ml_features']['duration']:
            ...
            for group in attack_groups.values():
                ...
                if candidates['highest_confidence']:
                    rep_flow = candidates['highest_confidence'].copy()
                    rep_flow['selection_reason'] = 'highest_confidence'
                    representative_flows.append(rep_flow)
                
                # 2. Median confidence
                if candidates['median_confidence']:
                    sorted_flows = sorted(
                        candidates['median_confidence'],
                        key=lambda x: x['confidence']
                    )
                    median_idx = len(sorted_flows) // 2
                    rep_flow = sorted_flows[median_idx].copy()
                    rep_flow['selection_reason'] = 'median_confidence'
                    representative_flows.append(rep_flow)
                
                # 3. Highest volume
                if candidates['highest_volume']:
                    rep_flow = candidates['highest_volume'].copy()
                    rep_flow['selection_reason'] = 'highest_volume'
                    representative_flows.append(rep_flow)
                
                # 4. Longest duration
                if candidates['longest_duration']:
                    rep_flow = candidates['longest_duration'].copy()
                    rep_flow['selection_reason'] = 'longest_duration'
                    representative_flows.append(rep_flow)
                
                group['representative_flows'] = representative_flows
                del group['flow_candidates']
                
                # Serialize sets
                group['protocols'] = list(group['protocols'])
                group['services'] = list(group['services'])
                group['unique_dst_ip_count'] = len(
                    group['target_ips_tracking']
                )
                group['unique_dst_port_count'] = len(
                    group['target_ports_tracking']
                )
                ...
                if group['flow_count'] > 0:
                    group['avg_duration'] = (
                        group['sum_duration'] / group['flow_count']
                    )
                    
                    if group['total_orig_bytes'] > 0:
                        group['amplification_ratio'] = (
                            group['total_resp_bytes'] /
                            group['total_orig_bytes']
                        )
                    else:
                        group['amplification_ratio'] = 0.0
                if group['anomaly_flow_count'] > 0:
                    group['avg_anomaly_score'] = (
                        group['sum_anomaly_score'] / group['anomaly_flow_count']
                    )
                    group['anomaly_ratio'] = (
                        group['anomaly_flow_count'] / group['flow_count']
                    )
                    group['is_anomaly_in_window'] = True
                else:
                    group['avg_anomaly_score'] = 0.0
                    group['anomaly_ratio'] = 0.0
                    group['is_anomaly_in_window'] = False # Trường mới để cảnh báo nhanh
                ...
                self.producer.produce(
                    OUTPUT_TOPIC,
                    json.dumps(group).encode('utf-8')
                )
                alerts_sent += 1
                
                # XAI queue for high severity
                xai_payload = {
                    ...
                    'representative_flows': group['representative_flows'],
                    'timestamp': group['timestamp']
                }
                
                self.producer.produce(
                    XAI_QUEUE_TOPIC,
                    json.dumps(xai_payload).encode('utf-8')
                )
                xai_queued += 1
                
                logger.warning(
                    f"⚡ HIGH SEVERITY: {group['attack_type']} from "
                    f"{group['src_ip']} (score={group['severity_score']}) "
                    f"→ XAI queued"
                )
            ...
            self.stats['xai_queued'] += xai_queued
            
            # Summary logging
            total_attacks = sum(g['flow_count'] for g in attack_groups.values())
            
            if total_attacks > 0:
                logger.warning(
                    f"🚨 ATTACKS: {total_attacks} flows → {alerts_sent} alerts | "
                    f"Benign: {benign_count} | XAI: {xai_queued}"
                )
            else:
            ...
            if self.stats['windows_processed'] % 10 == 0:
                ...
                logger.info(
                    f"📊 Stats: {self.stats} | "
                    f"Cache: {self.attack_cache.get_summary()}"
                )
