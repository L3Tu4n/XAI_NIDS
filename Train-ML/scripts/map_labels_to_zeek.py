...
# Label encoding (CẬP NHẬT: Thêm biến thể 2 khoảng trắng để khớp với log thực tế)
    ...
    def map_labels_simple(self, zeek_df, cic_df):
        ...
        zeek_df['attack_type'] = zeek_df['attack_type'].fillna('BENIGN')
        
        # 5. Encode Labels
        zeek_df['label'] = zeek_df['attack_type'].map(self.LABEL_ENCODING)
        zeek_df['label'] = zeek_df['label'].fillna(0)
    ...
    def label_normal_only(self, zeek_df):
        """
        Gán nhãn BENIGN cho dữ liệu normal-only (dùng cho Isolation Forest)
        """
        print("[*] Labeling normal-only dataset for Isolation Forest...")

        zeek_df = zeek_df.copy()
        zeek_df['attack_type'] = 'BENIGN'
        zeek_df['label'] = 0

        print(f"[✓] Normal-only dataset labeled: {len(zeek_df)} records")
        return zeek_df

if __name__ == "__main__":
    # Paths
    ZEEK_PARQUET = "./CIC-IDS-2017/conn_for_semi_supervised.parquet"
    CIC_CSV_DIR = "./CIC-IDS-2017/CSVs"  # Directory chứa CSV files
    OUTPUT_FILE = "./CIC-IDS-2017/labeled_conn_for_semi_supervised.parquet"
    ...
    INPUT_NORMAL_FILE = "./CIC-IDS-2017/conn_normal_only.parquet"
    OUTPUT_NORMAL_FILE = "./CIC-IDS-2017/labeled_conn_normal_only.parquet"

    print("[*] Loading normal-only Zeek data...")
    df = pd.read_parquet(INPUT_NORMAL_FILE)
    print(f"[✓] Loaded {len(df)} records")

    mapper = CICIDSLabelMapper(csv_dir=None)
    df_labeled = mapper.label_normal_only(df)

    print(f"[*] Saving labeled normal-only data to {OUTPUT_NORMAL_FILE}")
    df_labeled.to_parquet(OUTPUT_NORMAL_FILE, index=False, compression='snappy')
